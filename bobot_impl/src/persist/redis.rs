...
impl RedisStr {
    ...
    pub async fn new_async<T: Serialize + Send + 'static>(val: T) -> Result<Self> {
        tokio::spawn(async move {
            let v: Result<Self> = Ok(RedisStr(serde_cbor::to_vec(&val)?));
            v
        })
        .await?
    }
    ...
    where
        T: DeserializeOwned,
    {
        let res: T = serde_cbor::from_slice(self.0.as_slice())?;
        Ok(res)
    }
}
...
impl RedisPool {
        ...
        R: DeserializeOwned + Send + Sync,
    {
        ...
        conn.lrange::<&str, Vec<Vec<u8>>>(key, 0, -1)
            .await?
            .into_iter()
            .map(|v| serde_cbor::from_slice(&v.as_slice()).map_err(|e| anyhow!(e)))
            .collect()
    }
